...
1. senshi.binファイルの後ろに使用するROMファイル(128K以下)を連結します。

unixのcatコマンドを使用する例
$ cat senshi.bin game.rom > firmware.bin

windowsのcopyコマンドを使用する例
>copy /b senshi.bin + game.rom firmware.bin
...
RESETスイッチ設定
ONにしてください(何か問題が起きる場合はOFFにして下さい。コールドスタートが失敗する場合はP6本体のリセットボタンを押してリセットして下さい。RESETスイッチをOFFにするとROMバンクのリセットが効かなくなるのでゲームを再起動する場合はP6本体の電源OFF/ON操作を行なってください。)
